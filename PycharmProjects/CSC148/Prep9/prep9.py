...
class BinarySearchTree:
    ...
    def count(self, item: Any) -> int:
        ...
        It is expected that similar items are directly linked together?
    ...
    def smaller(self, item: Any) -> List:
        ...
        [2, 3, 5, 7, 9, 11]
        """
        # traverse to node <= than item
        # call _left.items() for that node
        if self.is_empty():
            return []
        elif self._root < item:
            item_list = self._left.items() + [self._root]
            item_list.extend(self._right.smaller(item))
            return item_list
        else:
