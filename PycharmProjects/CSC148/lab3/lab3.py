...
class user_player(Player):
    ...
    def move(self, current: int, min_step: int, max_step: int, goal: int) -> int:
        input_num = input(f'Please enter a number between {min_step} and {max_step}')
        while input_num < min_step or input_num > max_step:
                input_num = input(f'Please enter a number between {min_step} and {max_step}')
        return input_num
...
def make_player(generic_name: str, type: str) -> Player:
...
def main() -> None:
    ...
    p1 = make_player('p1', 'random')
    p2 = make_player('p2', 'user')
    while True:
