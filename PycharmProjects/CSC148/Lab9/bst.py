...
class BinarySearchTree:
    ...
    def items(self) -> List:
        ...
        if self.is_empty():
            ...
            current_list = [self._root]
            return self._left.items() + current_list + self._right.items()
    ...
    def height(self) -> int:
        ...
        >>> bst.height()
        2
        >>> bst._left._left = BinarySearchTree(10)
        >>> bst.height()
        3
        """
        if self.is_empty():
            ...
            count = 1
            count += max([self._left.height(), self._right.height()])
        return count
    ...
    def items_in_range(self, start: Any, end: Any) -> List:
        ...
        if self.is_empty():
            return []
        # check start and end conditions for root
        if self._root < start:
            return self._right.items_in_range(start, end)
        elif self._root > end:
            return self._left.items_in_range(start, end)
        elif start <= self._root <= end:
            # check start and end conditions for left and right
            tree_list = [self._root]
            left_list = self._left.items_in_range(start, end)
            right_list = self._right.items_in_range(start, end)
            return left_list + tree_list + right_list
    ...
    def rotate_right(self) -> None:
        ...
        <BLANKLINE>
        """
        # TODO: implement this method - Find efficient way to do this
        # What were the steps here?

        root_root, root_right, pivot = self._root, self._right, self._left
        self._root, self._left, self._right = pivot._root, pivot._left, BinarySearchTree(root_root)
        self._right._right = root_right
        self._right._left = pivot._right
    ...
    def rotate_left(self) -> None:
        ...
        """
        # TODO: implement this method!
        root_root, pivot, root_left = self._root, self._right, self._left
        self._root, self._left, self._right = pivot._root, BinarySearchTree(root_root), pivot._right
        self._left._right = pivot._left
        self._left._left = root_left
