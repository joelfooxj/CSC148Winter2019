...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        False
        """
        flag = self.values[0].evaluate()
        if self.op == 'and':
            for val in self.values:
                flag = flag and val.evaluate()
            return flag
        elif self.op == 'or':
            for val in self.values:
                flag = flag or val.evaluate()
            return flag
        else:
            raise ValueError(f'Invalid operator {self.op}')
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        flag = True
        first_val = self.left.evaluate()
        for val in self.comparisons:
            if val[0] == '<=':
                flag = flag and first_val <= val[1].evaluate()
            elif val[0] == '<':
                flag = flag and first_val < val[1].evaluate()
            if not flag:
                return False
            first_val = val[1].evaluate()
        return flag
