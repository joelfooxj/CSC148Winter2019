...
class RecursiveList:
    ...
    def _insert_first(self, item: Any) -> None:
        ...
        # oldlst = self
        # The above doesn't work. Self-referential
    ...
    def map(self, f: Callable[[Any], Any]) -> RecursiveList:
        ...
        '5 -> 7'
        """
        # forgot the terminating case
        new_rec = RecursiveList([])
        # if not self.is_empty():
        #     new_rec._first = f(self._first)
        #     new_rec._rest = self._rest.map(f)
        new_rec._first = f(self._first)
        if self._rest:
            new_rec._rest = self._rest.map(f)
        else:
            new_rec._rest = RecursiveList([])
        return new_rec

        # mutating the sturcture must maintain that structure
        # the methods that have been specified for this recursive structure
        # will only work if the structure remains intact as is.
