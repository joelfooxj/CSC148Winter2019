    ...
    for cust in log['customers']:
        ...
        for line in cust['lines']:
            if line['contract'] == 'prepaid':
                ...
                raise Exception("ERROR: Unknown contract type")

            new_line = PhoneLine(line['number'], contract)
            customer.add_phone_line(new_line)
...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        if event_data['type'] == 'call':
            ...
            new_call = Call(src_num, dst_num, time, duration, src_loc, dst_loc)
            if current_time.month > time.month or current_time.year > time.year:
