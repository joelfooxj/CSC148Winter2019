...
from linked_list_new import LinkedList_new
...
if __name__ == '__main__':
    for list_class in [LinkedList, LinkedList_new]:
        link_time = []
        # Try each list size
        for s in SIZES:
            time = profile_len(list_class, s)
            print(f'[{list_class.__name__}] Size {s:>6}: {time}')
            link_time.append(time)
        link_new_time = []
        for s in SIZES:
            time = profile_len(list_class, s)
            print(f'[{list_class.__name__}] Size {s:>6}: {time}')
            link_new_time.append(time)

    plt.plot(SIZES, link_time, SIZES, link_new_time)
