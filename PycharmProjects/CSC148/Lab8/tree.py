...
class Tree:
    ...
    def branching_factor(self) -> float:
        ...
        number of non-leaf nodes/number of children for all nodes
        ...
        if self.is_empty() or self._subtrees == []:
            ...
            return self.branches()/self.internal_values()
    ...
    def branches(self) -> int:
        ...
        if self.is_empty() or self._subtrees == []:
            ...
            for subtree in self._subtrees:
                branches += 1
                branches += subtree.branches()
            return branches

    def internal_values(self) -> int:
        """
        Returns the number of internal values in a tree
        >>> t = Tree(1, [Tree(2, []), Tree(5, [])])
        >>> t.internal_values()
        1
        >>> lt = Tree(2, [Tree(4, []), Tree(5, [])])
    ...
    def items_at_depth(self, d: int) -> List:
        ...
        >>> t2.items_at_depth(1)
        [5]
        >>> t3 = Tree(5, [Tree(6, [Tree(8, []), Tree(9, []), Tree(10, [])]), Tree(7, [Tree(11, []), Tree(12, []), Tree(13, [])])])
        >>> t3.items_at_depth(3)
        [8, 9, 10, 11, 12, 13]
        """
        if self.is_empty():
            ...
            value_list = []
            for subtree in self._subtrees:
                value_list.extend([subtree._root])
            return value_list
        else:
            d -= 1
            value_list = []
            for subtree in self._subtrees:
                value_list.extend(subtree.items_at_depth(d))
            return value_list
