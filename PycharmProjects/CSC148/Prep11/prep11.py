...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) < 2:
        ...
        partition = len(lst) // 3
        lst1 = lst[:partition]
        lst2 = lst[partition:partition*2]
        lst3 = lst[partition*2:]
        lst1_sorted = mergesort3(lst1)
        lst2_sorted = mergesort3(lst2)
        lst3_sorted = mergesort3(lst3)
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    # Need to take empty lists into account

    while index1 < len(lst1) and index2 < len(lst2) and index3 < len(lst3):
        if lst1[index1] <= lst2[index2] <= lst3[index3]:
            merged.append(lst1[index1])
            index1 += 1
        elif lst2[index2] <= lst1[index1] <= lst3[index3]:
            merged.append(lst2[index2])
            index2 += 1
        else:
...
def kth_smallest(lst: List, k: int) -> Any:
    ...
    10
    >>> kth_smallest([20, 10, -3, 4], 2)
    10
    >>> kth_smallest([-4, 3, 10, 20], 2)
    10
    """
    ...
    if k < 0 or k >= len(lst):
        ...
        if len(smaller) > k:
            ...
            return kth_smallest(bigger, k - 1)
